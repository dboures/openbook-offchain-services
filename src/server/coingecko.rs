...
pub fn service() -> Scope {
    web::scope("/coingecko").service(pairs)
    // .service(tickers)
    // .service(orderbook)
}
...
pub async fn pairs(context: web::Data<WebContext>) -> Result<HttpResponse, ServerError> {
    ...
    let pairs = markets
        .iter()
        .map(|m| CoinGeckoPair {
            ticker_id: m.market_name.clone(),
            base: m.base_mint.clone(),
            target: m.quote_mint.clone(),
            pool_id: m.market_pk.clone(),
        })
...
//     let market_names = markets.iter().map(|x| &x.market_name).collect();
//     let market_addresses = markets.iter().map(|x| &x.market_pk).collect();
...
//             CoinGeckoTicker {
//                 ticker_id: m.market_name.clone(),
//                 base_currency: m.base_mint.clone(),
//                 target_currency: m.quote_mint.clone(),
